...
type RootStackParamList = {
  ...
  PostView: { post: any; activeHashtag?: string };
  SearchScreen: { initialQuery?: string; initialType?: string; activeHashtag?: string };
};
...
const SearchScreen: React.FC<SearchScreenProps> = ({ route }) => {
  ...
  const [searchType, setSearchType] = useState<string>(route?.params?.initialType || "all");
  const [results, setResults] = useState<SearchResult>({ users: [], posts: [], hashtags: [], total: 0 });
  const [loading, setLoading] = useState<boolean>(false);
  const [trendingHashtags, setTrendingHashtags] = useState<any[]>([]);
  const [searchHistory, setSearchHistory] = useState<any[]>([]);
  const [showHistory, setShowHistory] = useState<boolean>(true);
  const [activeHashtag, setActiveHashtag] = useState<string | null>(route?.params?.activeHashtag || null);
  ...
  const renderPostResult = ({ item }: { item: any }) => {
    ...
    if (item.videos && item.videos.length > 0) {
      console.log(`Attempting to play video: ${item.videos[0]}`);
    }
        ...
        <View style={styles.actionsContainer}>
          <View style={styles.actionButtons}>
            ...
            <TouchableOpacity
              onPress={(e) => {
                e.stopPropagation();
                handleLikePress(item._id);
              }}
              style={styles.actionButton}
            >
              <Heart size={20} color={item.likes?.includes(user._id) ? colors.primary : colors.icon} />
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.background, borderBottomColor: colors.border }]}>
        ...
        <View style={[styles.tabContainer, { backgroundColor: colors.background }]}>
          ...
          <TouchableOpacity
            style={[styles.tab, activeTab === "trending" && { borderBottomColor: colors.primary, borderBottomWidth: 2 }]}
            onPress={() => {
        ...
        {activeTab === "search" ? (
          <>
            ...
            {!showHistory && !loading && results.total === 0 && searchQuery.trim() && (
              <View style={styles.emptyContainer}>
                <Text style={[styles.emptyText, { color: colors.text }]}>No results found for "{searchQuery}"</Text>
              </View>
            )}
          </>
        ) : (
